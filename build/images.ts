...
export default class Images {
  async generateSourceSetAsync(
    ...
    for (let i = 0; i < imageResult.length; i++) {
      ...
      responsiveImage.sizedImages.sort((a, b) =>
        a.size.width < b.size.width ? 1 : -1
      );
  ...
  async generateResponsiveImagesAsync(
    config: ImageGenerateConfig
    ...
    if (config.imageRoot) {
      config.images.forEach(
        (x, index) => (config.images[index] = path.join(config.imageRoot, x))
      );
    }
    ...
    for (const file of config.images) {
      ...
      for (let size of config.sizes) {
        sizedImages.push(
          await this.generateResponsiveImageAsync(
            file,
            size,
  ...
  async generateResponsiveImageAsync(
    file: string,
    size: Size,
    destinationDirectory: string,
    sharpConfig: SharpConfig
  ): Promise<{ file: string; size: Size }> {
    ...
    const validateWidthHeight = Images.validateWidthHeight(
      width,
      height,
      sharpConfig,
      metadata,
    ...
    try {
      ...
      if (sharpConfig.sharpen) {
        image.sharpen(
          sharpConfig.sharpen.sigma,
          sharpConfig.sharpen.flat,
          sharpConfig.sharpen.jagged
        );
      }
  ...
  private static validateWidthHeight(
    width: number,
    height: number,
    sharpConfig: SharpConfig,
    ...
    if (
      !(
        sharpConfig.resizeOptions.withoutEnlargement &&
