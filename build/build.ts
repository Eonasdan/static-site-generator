  ...
  async updatePostsAsync() {
    ...
    for (const file of posts) {
      ...
      if (postMeta.mastImage) {
        ...
        const fullyQualifiedImage = `${this.baseUrl}${
            postMeta.mastImage.getElementsByTagName('source')[0].srcset
        }`;
        metaImage = fullyQualifiedImage;
        this.setStructuredData(structuredData, 'image', [fullyQualifiedImage]);
      } else {
...
<priority>0.80</priority>
    ...
    const search = this.postsMeta
        .map((x) => x.toSearch(this.cleanText))
        .sort((a, b) => {
  ...
  async saveAsync(
    ...
    try {
      ...
      const thumbnailName = `${Utilities.slugify(
          thumbnailAlt || path.parse(thumbnail).name
      )}${path.extname(thumbnail)}`;
