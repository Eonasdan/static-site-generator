...
export default class Build {
  ...
  constructor() {
    this.siteConfig = JSON.parse(
      readFileSync(`./build/site-config.json`, 'utf8')
    );
    FileHelpers.siteConfig = this.siteConfig;
    this.subFolder = this.siteConfig.site.subfolder
      ? `${this.siteConfig.site.subfolder}/`
      : '';
  ...
  async updateAllAsync() {
    ...
    this.reset();
    await FileHelpers.removeDirectoryAsync(
      `./${this.siteConfig.output.main}`,
      false
    );
  ...
  loadTemplateAsync(template) {
    return fs.readFile(
      `./${this.siteConfig.source}/templates/${template}.html`,
      'utf8'
    );
  }
...
<html lang="en">${html}
  ...
  async prepareCssAsync() {
    ...
    const compileResult = await sass.compileAsync(
      `./${this.siteConfig.source}/styles/style.scss`,
      { sourceMap: true }
    );
  ...
  async postDocumentAsync() {
    const indexDocument = new JSDOM(
      await this.loadTemplateAsync('post-template')
    ).window.document;
    const shell = this.shellDocument;
    shell.getElementById('mainContent').innerHTML =
  ...
  async updatePostsAsync() {
    ...
    const posts = (
      await fs.readdir(`./${this.siteConfig.source}/partials`)
    ...
    for (const file of posts) {
      ...
      const postDocument = new JSDOM(await fs.readFile(fullPath, 'utf8')).window
        .document;
      ...
      newPageDocument.getElementById('post-inner').innerHTML =
        article.innerHTML;
      ...
      if (postMeta.mastImage) {
        this.setInnerHtml(
          newPageDocument.getElementById('post-thumbnail'),
          postMeta.mastImage
        );
        this.setInnerHtml(
          loopDocument.getElementsByClassName('post-thumbnail')[0],
      ...
      if (postMeta.metaImage)
        metaImage = `${this.baseUrl}${postMeta.metaImage}`;
      else {
        const mastImageHtml = this.htmlToElement(postMeta.mastImage);
        metaImage = `${this.baseUrl}${
          mastImageHtml.getElementsByTagName('source')[0].srcset
        }`;
      }

      if (metaImage) {
        this.setStructuredData(structuredData, 'image', [metaImage]);
      }
      ...
      (
        loopDocument.getElementsByClassName('post-link')[0] as HTMLLinkElement
      ).href = postMeta.url;
      ...
      this.setInnerHtml(
        ...
        Utilities.formatter.format(publishDate)
      );
      ...
      this.setStructuredData(structuredData, 'keywords', postMeta.tags);

      this.setInnerHtml(
        loopDocument.getElementsByClassName('post-author')[0],
      ...
      const completeHtml = await this.createRootHtmlAsync(
...
<priority>0.80</priority>
    ...
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/js/search.json`,
  ...
  async updateHomepageAsync() {
    const indexDocument = new JSDOM(
      await fs.readFile(
        `./${this.siteConfig.source}/templates/index.html`,
        'utf8'
      )
    ).window.document;
    ...
    const completeHtml = await this.createRootHtmlAsync(
      shell.documentElement.innerHTML
    );
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/index.html`,
      completeHtml
    );
    dropCss({
      css: this.css,
  ...
  async update404Async() {
    const indexDocument = new JSDOM(
      await fs.readFile(
        `./${this.siteConfig.source}/templates/404.html`,
        'utf8'
      )
    ).window.document;
    const shell = this.shellDocument;
    ...
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/404.html`,
      await this.createRootHtmlAsync(shell.documentElement.innerHTML)
    );
    dropCss({
      css: this.css,
      html: completeHtml,
...
${this.siteMap}
    ...
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/sitemap.xml`,
      this.siteMap
    );
  }
  ...
  async updateCssAsync() {
    ...
    for (const x of (await fs.readdir(`./${this.siteConfig.source}/partials`))
      .filter((file) => path.extname(file).toLowerCase() === '.html')
      .map((file) => `./${this.siteConfig.source}/partials/${file}`)) {
      await gatherCssAsync(x);
    }

    for (const x of (await fs.readdir(`./${this.siteConfig.source}/templates`))
      .filter((file) => path.extname(file).toLowerCase() === '.html')
  ...
  async cleanCssAsync() {
    ...
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/css/style.css`,
      optimized
    );
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/css/style.min.css`,
      cleanedCss
    );
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/css/style.css.map`,
      this.sourceMap
    );
  }
  ...
  async minifyJsAsync() {
    ...
    const getJsAsync = async () => {
      ...
      const files = (await fs.readdir(`./${this.siteConfig.source}/js`)).filter(
        (file) =>
          path.extname(file).toLowerCase() === '.js' && !file.includes('.min.')
      );

      for (const file of files) {
        output +=
          (await fs.readFile(
            `./${this.siteConfig.source}/js/${file}`,
            'utf8'
          )) + '\r\n';
      }
    ...
    await FileHelpers.writeFileAndEnsurePathExistsAsync(
      `./${this.siteConfig.output.main}/js/bundle.min.js`,
      uglified.code
    );
  }
  ...
  async getPictureSet(
    ...
    return (
      await this.imageProcessor.generateSourceSetAsync(
        {
          images: [image],
          sizes: defaultSizes,
  ...
  htmlToElement(html) {
    return JSDOM.fragment(html).firstChild as HTMLElement;
  }

  async saveAsync(postMeta: PostMeta, thumbnailAlt: string, rawEditor) {
    ...
    try {
      ...
      if (postImages.length > 0) {
        for (const image of postImages) {
          ...
          const newName = `${Utilities.slugify(
            image.data.alt || path.parse(url).name
          )}${path.extname(url)}`;
          const oldName = path.basename(url);
          const newPath = `.${url.replace(oldName, newName)}`;
          await fs.copyFile(`.${url}`, newPath);

          const pictureSet = await this.getPictureSet(
            newPath,
            postImageCopyPath,
            postMeta.file,
            image.data.alt
      ...
      const thumbnailName = `${Utilities.slugify(
        thumbnailAlt || path.parse(postMeta.thumbnail).name
      )}${path.extname(postMeta.thumbnail)}`;
      const oldName = path.basename(postMeta.thumbnail);
      const newPath = postMeta.thumbnail.replace(oldName, thumbnailName);
      await fs.copyFile(postMeta.thumbnail, newPath);

      const thumbnailPictureSet = await this.getPictureSet(
        newPath,
        postImageCopyPath,
        postMeta.file,
        thumbnailAlt
      ...
      const socialMetaImage =
        await this.imageProcessor.generateResponsiveImageAsync(
          newPath,
          { width: '1200' },
          postImageCopyPath,
          Images.mergeConfig({
      ...
      let newPost = (await this.loadTemplateAsync('empty-post'))
        ...
        .replace(
          /==formatted-date==/g,
          Utilities.formatter.format(postMeta.postDate)
        )
        .replace(/==body==/g, body)
        .replace(/==thumbnail==/g, thumbnailPictureSet.pictureTag)
        .replace(
          /==metaImage==/g,
          `${this.baseImagePath}${postMeta.file}/${socialMetaImage.file}`
        )
        .replace(/==raw-post-date==/g, postMeta.postDate.toISOString())
        .replace(/==tags==/g, postMeta.tags.join(','))
