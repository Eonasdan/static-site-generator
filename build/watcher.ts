...
export class Watcher {
    ...
    async startAsync() {
        ...
        this.parvusServer = new ParvusServer({
            ...
            middlewares: [
                ...
                //todo remove
                {
                    middleware: async (req: IncomingMessage, res: ServerResponse) => {
                        const results = await fs.readFile('C:\\Users\\dev\\Desktop\\az-cog-search-result.json', 'utf8')
                        res.write(results);
                        res.end();
                    },
                    route: '/search-data'
    ...
    async editorSaveAsync(req: IncomingMessage, res: ServerResponse) {
        ...
        const fields: EditorModel = await new Promise(function (resolve, reject) {
            form.parse(req, function (err, fieldsArray, files) {
                ...
                Object.keys(fieldsArray).forEach(
                    (x) => (editorModel[x] = fieldsArray[x][0])
                );
        ...
        const result = await this.builder.saveAsync(
            new PostMeta({
                file: slug,
                title: fields.title,
                postDate: postDate.toISOString(),
                updateDate: postDate.toISOString(),
                excerpt: fields.excerpt,
                tags: fields.tags.replace(/, /g, ',').split(','),
                author: new PostAuthor(fields.postAuthorName, fields.postAuthorUrl),
                mastImage: fields.thumbnail
            }),
            fields.thumbnailAlt,
            editor
        );
    ...
    private startFileWatcher() {
        ...
        watcher.on('all', handleChange).on('ready', () => {
            console.clear();
            console.log(
                `Serving at http://localhost:${this.builder.siteConfig.server.port}`
            );
