...
class Editor {
  ...
  areYouSureModal = bootstrap.Modal.getOrCreateInstance(
    document.getElementById('areYouSureModal')
  );
  ...
  async readyAsync() {
    ...
    if (!this.config.services.languageTools)
      document.getElementById('runLt').classList.add('d-none');

    // noinspection JSValidateTypes
    Prism.plugins.autoloader.languages_path =
      'https://cdnjs.cloudflare.com/ajax/libs/prism/1.28.0/';
    ...
    this.fileReader.addEventListener(
      ...
      (e) => {
        ...
        this.img.onload = () => {
          ...
          if (this.img.width < 1200) this.thumbnailHelp.classList.add('show');
  ...
  registerEventListeners() {
    ...
    this.previewButton.addEventListener('click', () => {
      if (this.previewButton.innerText === 'Preview') {
        this.editor.save().then((data) => {
          document.getElementById('previewContent').innerHTML = eonasdan.parser(
            data.blocks
          );
          Prism.highlightAll();
        });
      ...
      this.previewButton.innerText =
        this.previewButton.innerText === 'Preview' ? 'Editor' : 'Preview';

      this.toggleShow(this.editorDiv, this.outputDiv);
    });

    document
      .getElementById('previewNewButton')
    ...
    this.clearButton.addEventListener('click', () => {
      ...
      this.areYouSureModalConfirm.addEventListener(
        'click',
  ...
  setupEditor() {
    const draftEditor = JSON.parse(
      localStorage.getItem('draft-editor') || '{}'
    );
  ...
  metaBlur(event) {
    ...
    switch (input.name) {
      case 'title':
        this.outputDiv.getElementsByClassName('post-title')[0].innerText =
          input.value;
        break;
      case 'thumbnail':
        ...
        break;
      case 'postDate':
        this.outputDiv.getElementsByClassName('post-date')[0].innerText =
          this.formatter.format(new Date(input.value));
        break;
      case 'tags':
        ...
        break;
      case 'postAuthorName':
        this.outputDiv.getElementsByClassName('post-author')[0].innerText =
          input.value;
        break;
      /*case 'postAuthorUrl':
  ...
  restoreForm() {
    ...
    [...this.form.elements].forEach((field) => {
      const value =
        data[field.name] || this.config.defaultAuthor[author(field.name)];
      if (value) {
        field.value = value;
        field.blur();
      }
  ...
  async savePost() {
    ...
    outputData.blocks.forEach(
      (x) => (x.data.text = this.languageTools.cleanupMarkers(x.data.text))
    );
    this.form.classList.add('was-validated');
    const formData = new FormData(this.form);
    formData.append('editor', JSON.stringify(outputData));

    const savingToast = new bootstrap.Toast(
      ...
      .forEach((block) => {
        this.languageTools
          ...
          .then(async (results) => {
            ...
            const currentData = (await this.editor.save()).blocks.find(
              (x) => x.id === block.id
            ).data.text;
            ...
            const existing = this.blockLanguageToolData.find(
              (x) => x.id === block.id
            );
  ...
  makeToast(body, style) {
    ...
    this.toastyContainer.getElementsByClassName('toast-body')[0].innerHTML =
      body;
